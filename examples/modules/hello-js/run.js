...
// full 0-127 hardware LED palette, resolved to RGBA — so a screen color
// here traces back to a real palette entry instead of a hand-copied RGB
// literal. Regenerate with `go run ./cmd/genpalette` after any change to
// the palette itself (rare — it's a fixed, SysEx-sourced hardware table).
// See docs/guides/writing-a-process-module.md's "Colors" section.
const PALETTE = JSON.parse(fs.readFileSync(path.join(__dirname, "palette.json"), "utf8"));

// paletteColor looks up a named entry (PALETTE.byName, e.g. "white", "sky")
// and returns it as the {R,G,B,A} shape a screen op's color param needs.
...
// paletteById looks up any raw 0-127 hardware index (PALETTE.byIndex),
...
// tags of its own).
function draw() {
  const white = paletteColor("white");
